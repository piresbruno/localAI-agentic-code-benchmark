...
export function mapError(err: unknown): { status: number; body: ApiErrorBody } {
  ...
  if (err instanceof ZodError) {
    ...
    return {
      status: 400,
      body: { error: { code: ERROR_CODES.VALIDATION, message: 'Request validation failed', details } },
    };
  }
  // Log server-side only; the response carries no internals (standards §3).
  console.error('Unhandled error:', err);
  return { status: 500, body: { error: { code: ERROR_CODES.INTERNAL, message: 'Internal server error' } } };
}
