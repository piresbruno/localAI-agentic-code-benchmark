...
Execute **top to bottom**. Each row links to its self-contained spec. **Spec v2** (reduced scope, 600–1,000 LOC, both original projects with UI) supersedes all earlier runs. Row 3 (`tripsplit`) is a **probe-tier** edition: 250–350 LOC, CLI-only (graded on CLI/UX per its spec §7), coverage gate 85%. Row 4 (`logsluice`) is a **probe-tier** TypeScript CLI: 300–450 LOC, contract-first parallel slices (graded on CLI/UX + layering per its spec §7), coverage gate 85%.
...
`max_agents` = maximum concurrent subagents observed in the run's harness telemetry; `—` when the harness does not expose it. Informational only — not a gate; compared within-project like the other metrics. It is meaningful only for specs designed for parallel slices (currently `logsluice`).
